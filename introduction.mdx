---
title: "Introduction"
description: "Welcome to SaludYa API - A modern medical appointment management system"
---

## Overview

SaludYa API is a REST API for managing medical appointments built with a clean layered architecture. The system follows the Controller → Service → Repository pattern, ensuring separation of concerns and maintainability.

<CardGroup cols={2}>
  <Card title="Quick Start" icon="rocket" href="/quickstart">
    Get up and running with SaludYa API in minutes
  </Card>
  <Card title="Architecture" icon="building" href="/architecture">
    Learn about the layered architecture pattern
  </Card>
  <Card title="API Reference" icon="code" href="/api/overview">
    Explore all available endpoints
  </Card>
  <Card title="Authentication" icon="lock" href="/authentication">
    Secure your API requests
  </Card>
</CardGroup>

## Key features

<CardGroup cols={2}>
  <Card title="Appointment Management" icon="calendar">
    Create, update, and manage medical appointments with ease
  </Card>
  <Card title="Patient Records" icon="user">
    Comprehensive patient management system
  </Card>
  <Card title="Doctor Scheduling" icon="user-doctor">
    Manage doctor profiles and availability
  </Card>
  <Card title="Clean Architecture" icon="diagram-project">
    Built with layered architecture for scalability and maintainability
  </Card>
</CardGroup>

## Core resources

SaludYa API provides four main resources:

- **Appointments** - Schedule and manage medical appointments
- **Patients** - Store and retrieve patient information
- **Doctors** - Manage healthcare provider profiles
- **Schedules** - Define doctor availability and working hours

## Next steps

<CardGroup cols={2}>
  <Card title="Get Started" icon="play" href="/quickstart">
    Follow our quickstart guide to make your first API call
  </Card>
  <Card title="Setup Guide" icon="gear" href="/guides/setup">
    Learn how to set up the API in your environment
  </Card>
</CardGroup>
